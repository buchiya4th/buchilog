...
export function getAllPostsData(): PostsData {
  ...
  return fileNames.map((fileName) => {
    ...
    const thumb = matterResult.data.image.replace(
      /(.+)(\.[^.]+$)/,
      "$1-thumb$2"
    );
...
function confirmExistsList(list: "category" | "tags") {
  ...
  const existingList = listInAllPosts.filter(
    (x, i, self) => self.indexOf(x) === i
  );
  const targetList: Categories | Tags =
    list === "category" ? categoryList : tagList;
 ...
 * Related posts list
 */
export function getRelatedArticleList(
  currentId: string,
  currentPostData: PostData
): PostsData {
  const ARTICLE_LIMIT = 3;
  const allPostsData = getAllPostsData();
  const matchTagsData = allPostsData.filter((everyPostData) =>
    everyPostData.tags.find((tag) => {
  ...
  const removeCurrentArticleData = allRelatedArticleData.filter(
    (item) => item.id !== currentId
  );
  const reduceLimitArticleData = removeCurrentArticleData.filter(
    (item, index) => index < ARTICLE_LIMIT
  );
...
export async function getPostData(id: string): Promise<PostData> {
  ...
  const { title, description, date, category, tags, image } =
