...
async function generateSiteMap() {
  ...
  const posts = postPaths.map((postPath) => {
    ...
    const url =
      process.env.NEXT_PUBLIC_DOMAIN + "/" + postPath.replace(".md", "");
    return formatData(url, lastModTime);
  });
  ...
  const categoriesPaths = (() => {
    ...
    return sortAllcategories.filter((item, index, array) => {
      return (
        array.findIndex((item2) => item.category === item2.category) === index
      );
    });
  })();
  const categories = categoriesPaths.map((post) =>
    formatData(
  ...
  const tags = tagsPaths.map((post) =>
    formatData(`${process.env.NEXT_PUBLIC_DOMAIN}/tags/${post.tag}`, post.date)
  );
